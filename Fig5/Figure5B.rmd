...
compaired <- list(c("NR", "R"))
sname <- c("Cytotoxicity", "Helper", "Proliferation", "Terminal_exhaustion")
mycol <- c("#EABF00", "#2874C5") %>% rev()
data <- read.csv("Figure5B.csv")

FUN <- function(i) {
  ggplot(data, aes_string(x = "Response", y = i, fill = "Response")) +
...
l <- lapply(sname, FUN)
p2 <- cowplot::plot_grid(plotlist = l, nrow = 1, align = "hv")
```
